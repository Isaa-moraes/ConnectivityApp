...
import { StyleSheet, Text, View, ScrollView, Platform, TouchableOpacity, ActivityIndicator, FlatList } from 'react-native';
import * as Network from 'expo-network';
import NetInfo from '@react-native-community/netinfo';
import { Ionicons } from '@expo/vector-icons';
import { LinearGradient } from 'expo-linear-gradient';
import { Stack } from 'expo-router'; // Para remover a barra superior
...
export default function ConnectivityDashboard() {
  ...
  //1. Função de Hardware: Busca o IP Local do aparelho
  const fetchIP = async () => {
    try {
  ...
  //2. Função de Diagnóstico: Teste de Latência Real
  const testLatency = async () => {
    setLoadingLatency(true);
    const start = Date.now();
    try {
      // Faz uma requisição leve para medir o tempo de resposta
  ...
  //3. Função de Simulação de Hardware: Varredura de Redes
  const scanWifiNetworks = () => {
    setScanningWifi(true);
    setWifiList([]);

    // Simula o tempo de processamento do chip de rádio
    setTimeout(() => {
      const mockNetworks = [
  ...
  return (
    <LinearGradient colors={['#F8FAFC', '#F1F5F9']} style={styles.container}>
      {/* 4. Remove a barra superior padrão para um visual imersivo */}
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.grid}>
          ...
          <View style={styles.detailCard}>
            ...
            <Text style={styles.detailValue}>{conn.isInternetReacheable ? "OK": "Falha"}</Text>
          </View>
        </View>

        {/* BOTÃO DE AÇÃO: DIAGNÓSTICO */}
        <TouchableOpacity
        style={[styles.button, loadingLatency && { opacity: 0.7 }]}
        onPress={testLatency}
        disabled={loadingLatency}
        >
          {loadingLatency ? <ActivityIndicator color="#FFF" /> : <Text
          style={styles.buttonText}>TESTAR LATÊNCIA</Text>}
        </TouchableOpacity>

        { /* BOTÃO DE AÇÃO: VARREDURA */}
        <TouchableOpacity
        style={[styles.button, styles.btnScan, scanningWifi && {opacity: 0.7}]}
        onPress={scanWifiNetworks}
        disabled={scanningWifi}
        >
          {scanningWifi ? <ActivityIndicator color="#FFF" /> : <Text
          style={styles.buttonText}>VARREDURA DE REDES</Text>}
        </TouchableOpacity>
